...
def filter_big_scc(g,edges_to_be_removed):
	...
	sub_graphs = filter(lambda scc: scc.number_of_nodes() >= 2, [g.subgraph(ns) for ns in nx.strongly_connected_components(g)])
	return sub_graphs
...
def get_big_sccs(g):
	...
	edges_to_be_removed = []
	big_sccs = []
	for subnodes in nx.strongly_connected_components(g):
